...
디자인 시스템을 구축하면서 마지막까지 미뤄둔 컴포넌트가 하나 존재했습니다.
이 컴포넌트가 바로 **테이블 컴포넌트**입니다.

테이블의 컬럼별로 `boolean`, `status`, `badge` 등의 다양한 타입들이 존재했고, 각 타입마다 보여주는 방식이 달랐습니다. 그러면서도 지정된 타입 외에도 `custom` 타입을 추가로 받아 원하는 형태로 커스터마이징할 수 있는 기능도 있었습니다.
이것만 해도 복잡한데 이 테이블은 `수정 모드`, `무한 스크롤`, `페이지네이션 영역`, `카드 형태` 등을 지원해야 했습니다.

기존의 테이블 컴포넌트 경험을 살리면서도 성능과 확장성을 높이는 방향으로 어떤 과정을 통해 설계하고 개발했는지 소개하고자 합니다.

## 자주 사용하지 않는 기능은 과감히 분리하거나 버리자

모든 기능을 하나의 컴포넌트에 집어넣으려다 보니 코드의 응집도는 낮아지고 결합도만 높아졌습니다. 이를 해결하기 위해 가장 먼저 한 일은 실제 사용량을 분석하고 디자이너 분들과 논의하는 것이었습니다.
