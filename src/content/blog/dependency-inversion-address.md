...
> **작성 배경**
> 의존성 역전과 리팩토링의 의사결정과 결과를 설명하기 위해 AI와 함께 가상의 요구사항과 초기 코드를 만들었습니다. 이후 개선점을 찾아 단계별로 개선을 진행했습니다.

먼저 신입 개발자였던 과거의 저라면 이 배송지 입력 기능을 어떻게 구현했을지 떠올려 보겠습니다. 초기 요구사항만 보면 주소 검색 버튼으로 카카오 우편번호 서비스를 열고, 사용자가 선택한 주소가 배송 가능 지역인지 확인하면 충분해 보입니다.

이 방식으로도 기능은 동작하지만, 컴포넌트 안에는 UI 상태와 카카오 SDK 응답 처리, 배송 가능 지역을 판단하는 규칙, API 호출이 모두 섞이게 됩니다. 이 글에서는 이렇게 작성한 코드를 단계적으로 개선하면서, 주소 검색 구현이 달라져도 배송지 선택 규칙은 영향을 받지 않는 구조로 개선해 보겠습니다.

## 처음 받은 요구사항

예제의 최초 요구사항은 다음과 같습니다.
